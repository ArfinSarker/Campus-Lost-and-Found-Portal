...
public class ArchivedChatsActivity extends AppCompatActivity {
    ...
    private void setupSwipeRefresh() {
        swipeRefreshLayout.setColorSchemeResources(R.color.chat_list_loading_indicator);
        swipeRefreshLayout.setProgressBackgroundColorSchemeResource(R.color.chat_list_header_bar_bg);
    ...
    private void saveConversationsToCache(List<Conversation> list) {
        ...
        prefs.edit().putString("cached_archived_conversations", json).apply();
    }

    private void loadCachedConversations() {
        if (currentUnivId == null) return;
        android.content.SharedPreferences prefs = getSharedPreferences("ChatPrefs_" + currentUnivId.trim(), Context.MODE_PRIVATE);
        String json = prefs.getString("cached_archived_conversations", "[]");
        try {
    ...
    public void toggleArchiveConversation(String conversationId) {
        ...
        // Notify main tab/activity of badge & archived count updates
                ...
                public void onSuccess(java.util.List<java.util.Map<String, Object>> list) {
                    ...
                    SupabaseDatabaseHelper.update("messages", query, data, new SupabaseDatabaseHelper.DatabaseCallback<String>() {
                        @Override public void onSuccess(String result) {
                            UnreadBadgeHelper.sendBadgeUpdateBroadcast(ArchivedChatsActivity.this);
                        }
                        @Override public void onFailure(String errorMessage) {}
                    });
                }
    ...
    public void showConversationActionMenu(Conversation c, int adapterPosition) {
        ...
        // 2. Archive option (Unarchive text since it is already archived)
    ...
    private void updateEmptyState() {
        if (conversationList.isEmpty()) {
            if (isLoading) {
                ...
                tvEmptyTitle.setText("No Archived Chats");
                tvEmptyDesc.setText("Archived chats will remain hidden from the main chats list.");
    ...
    private static class ConversationsAdapter extends RecyclerView.Adapter<ConversationsAdapter.ViewHolder> {
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
            ...
            if (context instanceof ArchivedChatsActivity) {
                isPinned = ((ArchivedChatsActivity) context).isConversationPinned(c.getConversationId());
                isMuted = ((ArchivedChatsActivity) context).isConversationMuted(c.getConversationId());
            }
            ...
            if (context instanceof ArchivedChatsActivity) {
                isLocalUnread = ((ArchivedChatsActivity) context).isLocalUnread(c.getConversationId());
            }
            ...
            holder.itemView.setOnLongClickListener(v -> {
                if (context instanceof ArchivedChatsActivity) {
                    ((ArchivedChatsActivity) context).showConversationActionMenu(c, holder.getAdapterPosition());
                }
                return true;
            });
            ...
            holder.itemView.setOnClickListener(v -> {
                if (context instanceof ArchivedChatsActivity) {
                    ((ArchivedChatsActivity) context).markChatAsReadLocally(c.getConversationId());
                }
                ...
                if (context instanceof ArchivedChatsActivity) {
                    blockedByMe = ((ArchivedChatsActivity) context).blockedUserIds.contains(c.getOtherUserId());
                    blockedByOther = ((ArchivedChatsActivity) context).usersWhoBlockedMe.contains(c.getOtherUserId());
                }
