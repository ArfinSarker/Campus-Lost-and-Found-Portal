...
public class ClaimDetailsActivity extends AppCompatActivity {
    ...
    private TextView tvHeaderTitle, tvNameHeader, tvUniversityId, tvGender, tvBatch, tvLevelTerm, tvDepartment,
            tvSection, tvPhone, tvEmail, tvPreferredContact, tvDesignation, tvReportId;
    private TextView tvItemName, tvCategory, tvDescription, tvItemDetails, tvOwnershipVerification, tvHandlingStatus,
            tvSecurityQuestion;
    private TextView tvInformationLabel, tvContactLabel, tvItemHeaderLabel, tvClaimantRoleBadge;
    private View llContactSectionContainer, rowUniversityId, rowPhone, rowEmail;
    private ImageView ivClaimantPhoto, ivItemImage;
    private ViewPager2 viewPagerImageSlider;
    private TabLayout tabLayoutIndicator;
    private View llSection, llBatch, llLevelTerm, llDesignation, llDepartment, llPreferredContact;
    private View llHiddenItemDetails, llFloatingBadges;
    private LinearLayout llOwnershipVerification, llFoundSpecifics;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            senderId = savedInstanceState.getString("senderId");
            itemStatus = savedInstanceState.getString("itemStatus");
            notificationType = savedInstanceState.getString("notificationType");
            String itemJson = savedInstanceState.getString("currentItemJson");
            if (itemJson != null) {
                currentItem = new com.google.gson.Gson().fromJson(itemJson, Item.class);
            }
        } else {
        ...
        // Hide these explicitly to prevent any state restoration issues showing them
        if (llHiddenItemDetails != null) {
            llHiddenItemDetails.setVisibility(View.GONE);
        }
        if (llFloatingBadges != null) {
            llFloatingBadges.setVisibility(View.GONE);
        }
        ...
        // Make University ID copyable on long press (now on the row container)
        if (rowUniversityId != null) {
            rowUniversityId.setOnLongClickListener(v -> {
        ...
        // Make Phone Number interactive (now on the row container)
        if (rowPhone != null) {
            ...
            rowPhone.setOnLongClickListener(v -> {
                String phone = tvPhone.getText().toString().trim();
                if (!phone.isEmpty() && !"Not Specified".equalsIgnoreCase(phone)) {
                    android.content.ClipboardManager clipboard = (android.content.ClipboardManager) getSystemService(
                            android.content.Context.CLIPBOARD_SERVICE);
        ...
        // Make Email Address interactive (now on the row container)
        if (rowEmail != null) {
            ...
            rowEmail.setOnLongClickListener(v -> {
                String email = tvEmail.getText().toString().trim();
                if (!email.isEmpty() && !"Not Specified".equalsIgnoreCase(email)) {
                    android.content.ClipboardManager clipboard = (android.content.ClipboardManager) getSystemService(
                            android.content.Context.CLIPBOARD_SERVICE);
    ...
    private void initializeViews() {
        ...
        rowUniversityId = findViewById(R.id.rowUniversityId);
        ...
        llHiddenItemDetails = findViewById(R.id.llHiddenItemDetails);
        llFloatingBadges = findViewById(R.id.llFloatingBadges);
    }
    ...
    private void setupClaimantProfile(String claimantId) {
        setupClaimantProfile(claimantId, true);
    }

    private void setupClaimantProfile(String claimantId, boolean showError) {
            ...
            public void onSuccess(List<User> users) {
                if (users != null && !users.isEmpty()) {
                    ...
                    if (user != null) {
                        ...
                        boolean isStaffOrAdmin = "Staff".equalsIgnoreCase(userType)
                        ...
                        if (isStaffOrAdmin) {
                            tvDesignation
                                    .setText(user.getDesignation() != null ? user.getDesignation() : "Not Specified");
                        }
                        if (isStudent) {
        ...
        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
            ...
            public void run() {
                if (!isFinishing() && !isDestroyed()) {
                    setupClaimantProfile(senderId, false);
    ...
    private void fetchItemDetails(String itemId) {
        SupabaseDatabaseHelper.select("found_reports", "id=eq." + itemId + "&limit=1", new TypeToken<List<Item>>() {
            ...
            public void onSuccess(List<Item> items) {
                if (items != null && !items.isEmpty()) {
                    ...
                    SupabaseDatabaseHelper.select("lost_reports", "id=eq." + itemId + "&limit=1",
    ...
    private void displayItem(Item item) {
        ...
        String formattedId = ReportIdFormatter
                .format(item.getDisplayId() != null ? item.getDisplayId() : item.getReportId());
        ...
        tvPhone.setText(
                item.getUserPhone() != null && !item.getUserPhone().isEmpty() ? item.getUserPhone() : "Not Specified");
        tvPreferredContact
                .setText(item.getPreferredContactMethod() != null && !item.getPreferredContactMethod().isEmpty()
                        ? item.getPreferredContactMethod()
                        : "Not Specified");
        ...
        String formattedLocation = ReportLocationDisplay.formatFullLocation(item.getLocation(),
        ...
        if ("lost".equals(itemStatus)) {
            ...
            tvOwnershipVerification
                    .setText(item.getProofOfOwnershipDetail() != null && !item.getProofOfOwnershipDetail().isEmpty()
                            ? item.getProofOfOwnershipDetail()
                            : "Not Specified");
            ...
            if ("Claimed".equalsIgnoreCase(item.getAdminStatus())
            ...
            tvHandlingStatus.setText(item.getItemHandlingStatus() != null && !item.getItemHandlingStatus().isEmpty()
                    ? item.getItemHandlingStatus()
                    : "Not Specified");
            tvSecurityQuestion.setText(
                    item.getHiddenIdentificationQuestion() != null && !item.getHiddenIdentificationQuestion().isEmpty()
                            ? item.getHiddenIdentificationQuestion()
                            : "Not Specified");
    ...
    private void fetchReporterProfileForContact(Item item) {
        if (item == null || item.getUserId() == null)
            return;
        SupabaseDatabaseHelper.select("profiles", "university_id=eq." + item.getUserId() + "&limit=1",
    ...
    private void setupImageSlider(List<String> imageUrls, String fallbackUrl) {
        View.OnClickListener detailClickListener = v -> {
            if (currentItem != null) {
                ItemNavigationUtils.navigateToDetail(ClaimDetailsActivity.this, currentItem);
            }
        };
        ...
        if (imageUrls != null && imageUrls.size() > 1) {
            ...
            new TabLayoutMediator(tabLayoutIndicator, viewPagerImageSlider, (t, p) -> {
            }).attach();
        } else {
            ...
            if (url != null && !url.isEmpty()) {
                GlideApp.with(this).load(url).placeholder(R.drawable.ic_package).thumbnail(0.1f)
                        .diskCacheStrategy(DiskCacheStrategy.ALL).into(ivItemImage);
            } else {
                ivItemImage.setImageResource(R.drawable.ic_package);
            }
            ivItemImage.setOnClickListener(detailClickListener);
        }
    }

    private void markAsReturned() {
        if (itemId == null || senderId == null || itemStatus == null || currentUnivId == null)
            return;
        btnMarkReturned.setEnabled(false);
        ...
        SupabaseDatabaseHelper.update(table, "id=eq." + itemId, updates,
                new SupabaseDatabaseHelper.DatabaseCallback<String>() {
                    @Override
                    public void onSuccess(String result) {
                        // Fetch reporter details to send in confirmation notification
                        SupabaseDatabaseHelper.select("profiles", "university_id=eq." + currentUnivId + "&limit=1",
                                new TypeToken<List<User>>() {
                                }.getType(), new SupabaseDatabaseHelper.DatabaseCallback<List<User>>() {
                                    @Override
                                    public void onSuccess(List<User> reporters) {
                                        User reporter = (reporters != null && !reporters.isEmpty()) ? reporters.get(0)
                                                : null;
                                        String reporterName = (reporter != null) ? reporter.getName() : "A user";

                                        // Fetch recipient (the person who found/claimed) profile for their auth_id
                                        // (RLS)
                                        SupabaseDatabaseHelper.select("profiles",
                                                "university_id=eq." + senderId + "&limit=1",
                                                new TypeToken<List<User>>() {
                                                }.getType(), new SupabaseDatabaseHelper.DatabaseCallback<List<User>>() {
                                                    @Override
                                                    public void onSuccess(List<User> recipients) {
                                                        if (recipients != null && !recipients.isEmpty()) {
                                                            ...
                                                            String notificationId = java.util.UUID.randomUUID()
                                                                    .toString();
                                                            String itemName = tvItemName.getText().toString();
                                                            String type = "lost".equals(itemStatus) ? "item_claimed"
                                                                    : "item_return";
                                                            String message = "lost".equals(itemStatus)
                                                                    ? String.format(
                                                                            "\"%s\" has marked that they received \"%s\" from you. Click to view details.",
                                                                            reporterName, itemName)
                                                                    : String.format(
                                                                            "\"%s\" has marked that they returned \"%s\" from you. Click to view details.",
                                                                            reporterName, itemName);

                                                            Notification notification = new Notification(notificationId,
                                                                    senderId, currentUnivId,
                                                                    reporterName,
                                                                    (currentItem != null
                                                                            && currentItem.getUserPhone() != null
                                                                            && !currentItem.getUserPhone().isEmpty())
                                                                                    ? currentItem.getUserPhone()
                                                                                    : (reporter != null
                                                                                            ? reporter.getPhone()
                                                                                            : ""),
                                                                    (currentItem != null
                                                                            && currentItem.getUserEmail() != null
                                                                            && !currentItem.getUserEmail().isEmpty())
                                                                    ...
                                                                    reporter != null ? reporter.getProfileImageUrl()
                                                                            : "",
                                                                    itemId, itemName, message,
                                                                    System.currentTimeMillis(), type, "");
                                                            notification.setItemName(itemName); // Ensure itemName is
                                                                                                // explicitly set
                                                            notification.setUserId(recipientAuthId);

                                                            SupabaseDatabaseHelper.insert("notifications", notification,
                                                                    new SupabaseDatabaseHelper.DatabaseCallback<String>() {
                                                                        @Override
    ...
    protected void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        outState.putString("senderId", senderId);
        outState.putString("itemStatus", itemStatus);
        outState.putString("notificationType", notificationType);
        if (currentItem != null) {
            outState.putString("currentItemJson", new com.google.gson.Gson().toJson(currentItem));
        }
    }
